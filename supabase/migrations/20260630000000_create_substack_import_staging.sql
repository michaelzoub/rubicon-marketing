-- Private staging tables for creator-owned Substack exports.
...
create table if not exists public.creator_import_candidates (
  id text primary key,
  import_job_id text not null references public.creator_import_jobs(id) on delete cascade,
  creator_id text not null references public.creators(id) on delete cascade,
  ...
  original_html_private text not null default '',
  sanitized_html_private text not null default '',
  plain_text_private text not null default '',
  sections_json_private jsonb not null default '[]'::jsonb,
  public_index_json jsonb not null default '{}'::jsonb,
  recommended_price_per_word_cents numeric(10,3) not null,
  selected_price_per_word_cents numeric(10,3),
  estimated_max_price_cents numeric(14,3) not null,
  status text not null check (status in ('preview', 'imported', 'skipped', 'failed')),
  warning text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
