...
export function AgentDemo({
  ...
  const [minimal, setMinimal] = useState(minimalProp ?? embedded);
  ...
  useEffect(() => {
    ...
    setPaused(pausedProp ?? params.get("paused") === "true");
    setLoop(loopProp ?? params.get("loop") === "true");
    setReady(true);
...
function PaidStreamScene({ progress }: { progress: number }) {
  return (
    <SceneStage
      ...
      bg="🌊"
      segments={[
        { kind: "text", weight: 1.85, text: "Agents pay only for the information they read." },
...
function DemoStyles({ embedded = false }: { embedded?: boolean }) {
  return (
    <style>{`
      .dv-root {
      ...
      .dv-root--embed .dv-stage {
        height: 100%;
        padding: clamp(14px, 2.5vw, 28px) clamp(16px, 3vw, 36px);
        gap: 10px;
      }

      .dv-root--embed .dv-spotlight {
        min-height: 0;
      }

      .dv-root--embed .dv-scene {
        min-height: 0;
        height: 100%;
      }

      .dv-root--embed .dv-mainline {
        font-size: clamp(1.35rem, 2.8vw, 2rem);
      }

      .dv-root--embed .dv-scene-focal {
        max-width: min(100%, 640px);
      }

      .dv-root--embed .dv-scene-focal-wide {
